#!/bin/zsh
set -euo pipefail
SCRIPT_DIR=${0:A:h}
PROJECT_DIR=${SCRIPT_DIR:h}
FRONTEND_DIR="$PROJECT_DIR/frontend"
OUTPUT_DIR="$PROJECT_DIR/build"
APP_DIR="$OUTPUT_DIR/Ark Intelligence.app"
SIGNING_IDENTITY=${ARK_CODESIGN_IDENTITY:--}

require_app_stopped() {
  if pgrep -x ArkIntelligence >/dev/null 2>&1; then
    echo "请先完全退出 Ark Intelligence 再构建。覆盖正在运行的应用会导致进程与磁盘签名不一致，影响辅助功能等授权。" >&2
    exit 73
  else
    local probe_exit=$?
    if (( probe_exit != 1 )); then
      echo "无法检查 Ark 运行状态，已停止构建，未替换应用。" >&2
      exit 74
    fi
  fi
}

require_app_stopped

"$PROJECT_DIR/backend/.venv/bin/python" "$SCRIPT_DIR/sync_native_catalog.py"
swift build --package-path "$FRONTEND_DIR" --scratch-path "$OUTPUT_DIR/swift" -c debug -j 2
BIN_DIR=$(swift build --package-path "$FRONTEND_DIR" --scratch-path "$OUTPUT_DIR/swift" -c debug --show-bin-path)

require_app_stopped
if [[ -d "$APP_DIR" ]]; then
  rm -rf "$APP_DIR"
fi
mkdir -p "$APP_DIR/Contents/MacOS" "$APP_DIR/Contents/Resources"
cp "$BIN_DIR/ArkIntelligence" "$APP_DIR/Contents/MacOS/ArkIntelligence"
if [[ -d "$BIN_DIR/ArkIntelligence_ArkIntelligence.bundle" ]]; then
  cp -R "$BIN_DIR/ArkIntelligence_ArkIntelligence.bundle" "$APP_DIR/Contents/Resources/"
fi
cp "$FRONTEND_DIR/Info.plist" "$APP_DIR/Contents/Info.plist"
codesign --force --sign "$SIGNING_IDENTITY" --identifier com.arkintelligence.desktop "$APP_DIR"
if [[ "$SIGNING_IDENTITY" == "-" ]]; then
  echo "此构建使用临时签名。代码变更后辅助功能授权可能失效；请打开新版本，必要时在系统设置中移除旧条目并重新添加当前应用。有固定签名证书时可设置 ARK_CODESIGN_IDENTITY。" >&2
fi
echo "$APP_DIR"
